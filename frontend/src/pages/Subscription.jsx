...
const QR_WINDOW_SECONDS = 10 * 60 // 10 minutes
      ...
      <div className="text-center mb-12">
        ...
        {user?.subscription?.active && (
          <div className="mt-4 inline-block px-4 py-2 rounded-2xl bg-success/10 border border-success/30 text-success text-sm font-medium">
            ✅ Active plan: {user.subscription.plan} — renews {new Date(user.subscription.expiresAt).toLocaleDateString('en-IN')}
          </div>
        )}
      </div>
      ...
      {user && payments.length > 0 && (
        <div className="mt-14 max-w-2xl mx-auto">
          ...
          <div className="space-y-3">
            {payments.map((p) => (
              <div key={p.id} className="flex items-center justify-between border border-border rounded-2xl px-4 py-3 text-sm">
                <div>
                  <div className="font-semibold text-ink">{p.plan} — ₹{p.amount}</div>
                  <div className="text-ink-soft text-xs">UTR: {p.utr} · {new Date(p.createdAt).toLocaleString('en-IN')}</div>
                </div>
                <span
                ...
                >
                  {p.status === 'approved' ? '✅ Verified' : p.status === 'rejected' ? '✗ Rejected' : '⏳ Pending review'}
                </span>
              </div>
            ))}
          </div>
...
function PaymentModal({ plan, user, onClose }) {
  ...
  const [stage, setStage] = useState('scan') // scan -> confirm -> verifying -> done
  ...
  const upiUri = useMemo(() => buildUpiUri({
    amount: finalAmount,
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center px-4">
      <div className="absolute inset-0 bg-ink/50 backdrop-blur-sm" onClick={stage === 'verifying' ? undefined : onClose} />
      <div className="relative bg-white rounded-2xl shadow-lift max-w-md w-full p-7">
        {stage !== 'verifying' && (
          <button onClick={onClose} className="absolute top-4 right-4 text-ink-soft hover:text-ink text-lg">✕</button>
        )}

        {stage === 'scan' && (
          <>
            <h3 className="font-display font-bold text-xl text-ink mb-1">Pay via UPI</h3>
            <p className="text-sm text-ink-soft mb-4">{plan.name} plan — scan the QR with any UPI app (GPay, PhonePe, Paytm).</p>

            {!expired ? (
              <div className="flex flex-col items-center">
                <img src={qrImgSrc} alt="UPI QR code" className="w-56 h-56 rounded-xl border border-border" />
                <div className="mt-3 text-xs text-ink-soft">QR expires in <span className="font-mono font-semibold text-accent">{formatMMSS(secondsLeft)}</span></div>
              </div>
            ) : (
              <div className="flex flex-col items-center py-10">
                <div className="text-3xl mb-2">⏰</div>
                <p className="text-sm text-ink-soft mb-4">This QR session expired for security. Generate a new one to continue.</p>
                <button onClick={regenerate} className="px-5 py-2.5 rounded-2xl bg-accent text-white text-sm font-semibold hover:bg-accent-hover">
                  Generate new QR
                </button>
              </div>
            )}
            ...
            <div className="mt-5 bg-bg-soft rounded-2xl p-4 text-sm space-y-2">
              <div className="flex justify-between"><span className="text-ink-soft">UPI ID</span><span className="font-mono font-medium text-ink">{UPI_VPA}</span></div>
              <div className="flex justify-between"><span className="text-ink-soft">Payable amount</span><span className="font-semibold text-ink">₹{finalAmount}</span></div>
              {discountRupees > 0 && (
                <div className="flex justify-between text-success"><span>Points discount applied</span><span>−₹{discountRupees}</span></div>
              )}
            </div>
            ...
            {maxRedeemable >= POINTS_PER_RUPEE && (
              <div className="mt-4">
                ...
                />
                <div className="text-xs text-ink-soft mt-1">Redeeming {redeemPoints} pts → ₹{discountRupees} off</div>
              </div>
            )}
        ...
        {stage === 'confirm' && (
          <>
            ...
            <p className="text-sm text-ink-soft mb-4">
              Enter the UPI reference number (UTR) from your payment app's transaction receipt. We use this to match your payment to your account.
            </p>
            ...
            <div className="flex gap-3 mt-5">
              <button onClick={() => setStage('scan')} className="flex-1 py-2.5 rounded-2xl border border-border text-sm font-medium text-ink hover:bg-bg-soft">
                Back
              </button>
              <button onClick={handleSubmitUtr} className="flex-1 py-2.5 rounded-2xl bg-accent text-white text-sm font-semibold hover:bg-accent-hover">
                Submit
              </button>
            </div>
          </>
        ...
        {stage === 'done' && (
          <div className="py-8 flex flex-col items-center text-center">
            ...
            <p className="text-xs text-ink-soft mb-6">
              Your subscription activates once the payment is verified against the UPI account — usually within a few hours. Track its status below on this page.
            </p>
            <button onClick={onClose} className="px-6 py-2.5 rounded-2xl bg-accent text-white text-sm font-semibold hover:bg-accent-hover">
              Done
            </button>
          </div>
        )}
