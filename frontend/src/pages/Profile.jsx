    ...
    <div className="max-w-4xl mx-auto px-5 py-14">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          ...
          <p className="text-ink-soft text-xs mt-1">
            Account created {new Date(user.createdAt).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-8">
        <div className="border border-border rounded-2xl p-6 bg-white">
          ...
          <div className="flex items-baseline gap-2">
            ...
            <span className="text-sm text-ink-soft">day{(user.streakCurrent || 0) === 1 ? '' : 's'} current streak</span>
          </div>
          ...
          <p className="text-xs text-ink-soft/70 mt-3">
            Solve a problem or register for a contest each day to keep your streak alive. It updates automatically based on real activity dates — no manual resets.
          </p>
        </div>
        ...
        <div className="border border-border rounded-2xl p-6 bg-white">
          ...
          {user.subscription?.active ? (
            <>
              <p className="text-sm text-ink">Plan: <strong>{user.subscription.plan}</strong></p>
              <p className="text-xs text-ink-soft mt-1">Renews {new Date(user.subscription.expiresAt).toLocaleDateString('en-IN')}</p>
            </>
          ) : (
            <>
              <p className="text-sm text-ink-soft">You're on the Free plan.</p>
              <Link to="/subscription" className="inline-block mt-3 text-xs font-semibold text-accent hover:underline">Upgrade for more →</Link>
            </>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-2xl p-6 bg-white mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <Link to="/wallet" className="text-xs font-semibold text-accent hover:underline">Open Wallet →</Link>
        </div>
        <p className="text-sm text-ink-soft mb-3">You've referred {user.referralCount || 0} friend{(user.referralCount || 0) === 1 ? '' : 's'}. Share your link to earn more points.</p>
      ...
      <div className="border border-border rounded-2xl p-6 bg-white">
        ...
        {registrations.length === 0 ? (
          <p className="text-sm text-ink-soft">You haven't registered for any contests yet. <Link to="/contests" className="text-accent hover:underline">Browse contests →</Link></p>
        ) : (
          <ul className="space-y-2">
            {registrations.map((r) => (
              <li key={r.contestId} className="flex justify-between text-sm border-b border-border pb-2 last:border-0">
                <Link to={`/contests/${r.contestId}`} className="text-ink hover:text-accent font-medium">{r.contestId}</Link>
