    ...
    <div className="card-lift bg-white border border-border rounded-2xl overflow-hidden shadow-soft flex flex-col">
      ...
      <div className="p-5 flex flex-col gap-3 flex-1">
        ...
        <div className="flex flex-wrap gap-2 text-xs">
          <span className={`px-2.5 py-1 rounded-full font-medium ${difficultyColor[contest.difficulty]}`}>{contest.difficulty}</span>
          <span className="px-2.5 py-1 rounded-full bg-muted text-ink-soft font-medium">⏱ {contest.duration}</span>
          <span className="px-2.5 py-1 rounded-full bg-muted text-ink-soft font-medium">👥 {contest.participants.toLocaleString('en-IN')}</span>
        </div>
        ...
        <div className="flex gap-2 mt-auto pt-2">
          <Link to={`/contests/${contest.id}`} className="flex-1 text-center py-2.5 rounded-2xl border border-border text-sm font-medium text-ink hover:bg-bg-soft transition-colors">
            Details
          </Link>
          <button
