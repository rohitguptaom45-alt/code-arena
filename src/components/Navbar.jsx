  ...
  return (
    <header className="sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-border">
      <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
        <Link to="/" onClick={closeAllMenus} className="flex items-center gap-2 shrink-0">
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          <div className="relative" ref={moreRef}>
            ...
            {moreOpen && (
              <div className="absolute top-full mt-2 left-0 w-56 bg-white border border-border rounded-2xl shadow-lift p-2 z-50">
                {moreItems.map((item) => (
                  <Link
                    key={item.label}
                    to={item.to}
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-border bg-white px-5 py-3 space-y-1">
          ...
          <button
            ...
            className="w-full flex items-center justify-between px-3 py-2 rounded-xl text-sm text-ink-soft hover:bg-bg-soft"
          >
            More <span className={`transition-transform ${mobileMoreOpen ? 'rotate-180' : ''}`}>▾</span>
          </button>
          {mobileMoreOpen && (
