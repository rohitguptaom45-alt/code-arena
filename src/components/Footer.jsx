...
function LinkedInIcon({ className }) {
  return (
    <svg viewBox="0 0 24 24" className={className} fill="currentColor">
      ...
      <path
        d="M11.5 10.5h2.1v1.1c.5-.8 1.4-1.3 2.4-1.3 2 0 2.9 1.3 2.9 3.4v3.8h-2.2v-3.4c0-1-.4-1.7-1.3-1.7-.9 0-1.5.6-1.5 1.7v3.4h-2.4v-7Z"
        stroke="none"
      />
    </svg>
  )
...
const columns = [
  {
    title: 'Quick Links',
    links: ['Home', 'Contests', 'Leaderboard', 'Subscription'],
  },
  {
    title: 'Resources',
    links: ['API Documentation', 'Blog', 'Announcements', 'Download App'],
  },
  {
    title: 'Community',
    links: ['Discussions', 'Feedback', 'Report Bug', 'Developers'],
  },
  {
    title: 'Legal',
      ...
      <div className="max-w-7xl mx-auto px-5 py-14 grid grid-cols-2 md:grid-cols-5 gap-8">
        <div className="col-span-2">
          <div className="flex items-center gap-2 mb-3">
            <span className="w-9 h-9 rounded-2xl bg-accent text-white grid place-items-center font-mono font-bold text-sm">
              &lt;/&gt;
            </span>
            <span className="font-display font-bold text-lg">
              Code<span className="text-accent-soft">Arena</span>
            </span>
          </div>
          <p className="text-sm text-white/60 max-w-xs">
            Compete • Debug • Build • Win. A premium arena for developers to sharpen skills and win real prizes.
          </p>
        ...
        {columns.map((col) => (
          <div key={col.title}>
            ...
            <ul className="space-y-2">
              {col.links.map((l) => (
                <li key={l}>
                  <Link to="/more" className="text-sm text-white/60 hover:text-accent-soft transition-colors">
                    {l}
                  </Link>
                </li>
              ))}
