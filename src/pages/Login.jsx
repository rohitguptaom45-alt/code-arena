...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('')
    setSubmitting(true)
    const result = await loginUserRemoteFirst(email, password)
    setSubmitting(false)
    if (result.error) {
  ...
  return (
    <div className="min-h-[calc(100vh-64px)] grid md:grid-cols-2">
      <div className="hidden md:flex flex-col items-center justify-center bg-bg-soft p-12 relative overflow-hidden">
        ...
        <h2 className="font-display font-bold text-2xl text-ink text-center max-w-sm">Welcome back to the arena.</h2>
      ...
      <div className="flex items-center justify-center p-8">
        <form onSubmit={handleSubmit} className="w-full max-w-sm">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="w-full py-3 rounded-2xl bg-accent text-white font-semibold hover:bg-accent-hover transition-colors shadow-lift disabled:opacity-60"
          >
