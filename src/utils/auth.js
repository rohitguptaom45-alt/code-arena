...
export async function changeAvatarRemoteFirst(username, avatarId) {
  const local = updateStoredUser(username, {
    avatar: avatarId,
  })
  try {
...
export async function fetchCurrentUserRemote() {
  try {
    const res = await api.currentUser()
    if (res?.data) return upsertLocalFromRemote(res.data)
  } catch {}
...
export async function changePasswordRemoteFirst(oldPassword, newPassword) {
  try {
    ...
    return {
      error: err.message || "Couldn't reach the backend to change your password.",
    }
  }
}
...
export function resetPasswordLocal(email, newPassword) {
  ...
  if (!key)
    return {
      error: 'No account found with that email on this device.',
    }
  users[key] = {
    ...users[key],
